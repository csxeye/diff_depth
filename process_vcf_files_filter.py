...
for i in range(1,23):
    ...
    SNPinfo = open(str(i) + "_snp_info.csv","wb")
    ALIdata = open(str(i) + "_ALI_data.csv","wb")
    OTGdata = open(str(i) + "_OTG_data.csv","wb")
    ...
    ALIwriter.writerows([["Subject","0MA","1MA","2MA"]])

    OTGwriter = csv.writer(OTGdata,delimiter=",")
    OTGwriter.writerows([["Subject","0MA","1MA","2MA"]])
    ...
    while cont2:
        ...
        if (cur_ali.CHROM==cur_otg.CHROM) and (cur_ali.POS==cur_otg.POS):
            ...
            for sample in cur_ali.samples:
                total += 1
                if sample.data.PL is not None:
                    nm += 1
                    cur_ali_samples += [[sample.sample]+sample.data.PL]
            ...
            for sample in cur_otg.samples:
                total1 += 1
                if (sample.data.GL is not None) and (sample.sample in euros):
                    nm1 += 1
                    cur_otg_samples += [[sample.sample]+sample.data.GL]

            if float(nm1)/float(total1) <= 0.9:
                pass_filter = False
